...
const ScripturePanel: React.FC<ScripturePanelProps> = ({ setReadingContext, onAskAboutVerse }) => {
    ...
    const fetchScriptures = useCallback(async (volume: ScriptureVolume) => {
    ...
    useEffect(() => {
        fetchScriptures(selectedVolume);
    }, [selectedVolume, fetchScriptures]);
    ...
    const renderContent = () => {
        ...
        if (!scriptureData || !currentChapterData) return (
        ...
        return (
            <article className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8">
                ...
                <div className="text-lg leading-loose font-serif">
                    {currentChapterData.verses.length > 0 ? currentChapterData?.verses.map(verse => (
