...
import type { ViewMode, ChatMode, Message, Skill } from '../types';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  onInstallPWA, showInstallButton, activeSkill, onOpenSkillSelector, onOpenSearch
}) => {
  ...
  const actionItems = [
    ...
    { action: () => onOpenSearch?.(), label: 'Search Conversations', icon: <WebIcon /> },
      ...
      >
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
          <nav className="p-2">
            ...
            <div className="px-3 mt-4 mb-1">
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Agent Features</h3>
            </div>
            <ul>
              <li>
                <button onClick={() => { setActiveView('dashboard'); if (window.innerWidth < 768) onClose(); }} className={`w-full flex items-center gap-3 p-3 rounded-md text-left transition-colors ${activeView === 'dashboard' ? 'bg-blue-600/50 text-white' : 'text-gray-300 hover:bg-slate-700/50'}`}>
                  <span className="text-lg">📊</span><span>Study Dashboard</span>
                </button>
              </li>
              <li>
              ...
              <li>
                <button onClick={() => { setActiveView('reminders'); if (window.innerWidth < 768) onClose(); }} className={`w-full flex items-center gap-3 p-3 rounded-md text-left transition-colors ${activeView === 'reminders' ? 'bg-blue-600/50 text-white' : 'text-gray-300 hover:bg-slate-700/50'}`}>
                  <span className="text-lg">🔔</span><span>Reminders</span>
                </button>
              </li>
            </ul>
